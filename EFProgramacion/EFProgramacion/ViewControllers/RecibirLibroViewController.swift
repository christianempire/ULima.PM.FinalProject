...
class RecibirLibroViewController: UIViewController , AVCaptureMetadataOutputObjectsDelegate {
    ...
    func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        if (metadataObjects.count > 0 && metadataObjects.first is AVMetadataMachineReadableCodeObject) {
            ...
            session.stopRunning()
            // Metodo que verifica el QR (lo de abajo es del barcode para sacar, en este caso tendria que haber una operación que transfiera
            // la responsabilidad y eliminar el codigo qr anterior
